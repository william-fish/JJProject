...
def cleanup_daily_data(today: str):
    ...
    keep_dates = {today}
    if yesterday:
        keep_dates.add(yesterday)

    # 清理 item_data: {date: {uid: [items]}}
    if item_data:
        old_keys = [date for date in item_data.keys() if date not in keep_dates]
        if old_keys:
    ...
    if effects_data:
        old_keys = [date for date in effects_data.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if fortune_data:
        old_keys = [date for date in fortune_data.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if reset_blind_box_records:
        old_keys = [date for date in reset_blind_box_records.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if select_wife_records:
        old_keys = [date for date in select_wife_records.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if beat_wife_records:
        old_keys = [date for date in beat_wife_records.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if seduce_records:
        old_keys = [date for date in seduce_records.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if market_purchase_records:
        old_keys = [date for date in market_purchase_records.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if gift_requests:
        old_keys = [date for date in gift_requests.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
    ...
    if discarded_item_pools:
        old_keys = [date for date in discarded_item_pools.keys() if date not in keep_dates]
        if old_keys:
            for date in old_keys:
...
class WifePlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.status_effect_specs = [
            ...
            },
            {
                "id": "protect_from_ntr",
                "label": "纯爱战士",
                "desc": "纯爱战士：今日老婆不会被牛走，也无法使用「换老婆」；每次使用道具有概率获得「白月光」道具卡",
                "item_name": "纯爱战士",
            ...
            },
            {
                "id": "positive",
                "label": "阳性",
                "desc": "阳性：当你的道具或指令成功对其他人生效时，会将自身一个状态传染给对方（优先低星状态）；且每当你使用道具成功后，有35%概率随机失去一个状态（包括阳性状态本身，即有概率痊愈，且有50%概率优先失去高星状态）",
            ...
            },
            {
                ...
                "label": "盲盒爱好者",
                "desc": "盲盒爱好者：今日每2小时可免费抽一次盲盒并清空现有道具，抽盲盒时暴击率降低50%，且每3分钟仅能使用一个道具",
                "item_name": "囤囤鼠",
            ...
            },
            {
                "id": "magic_circuit",
                "label": "魔术回路",
                "desc": "魔术回路：当你今天背包里没有道具卡时，会立刻获得一张随机道具卡（每日最多触发20次）",
                "item_name": "trace-on",
            ...
            },
            {
                "id": "cheat",
                "label": "老千",
                "desc": "老千：你抽盲盒或使用道具时，30%概率（受概率效果影响）获得一张随机道具卡，且洗牌时获得的道具卡数量增加，且荷官状态计数需求降低",
                "item_name": "出千",
            ...
            },
            {
                "id": "alibi",
                "label": "神的不在场证明",
                "desc": "神的不在场证明：今日你对他人使用道具时，不会被记录为使用者",
                "item_name": "神的不在场证明",
                "checker": flag_checker("alibi"),
            },
            {
                "id": "gamble_multiplier",
                "label": "博弈",
                "desc": "博弈：你今日的概率判定受到博弈加成影响",
                "item_name": "博弈",
                "checker": meta_exists_checker("gamble_multiplier"),
                "desc_generator": lambda today, uid, gid: (
                    (lambda mult: (
                        f"博弈：概率加成{'增加' if mult > 1 else '减少'}了 {abs(int((mult - 1) * 100))}%"
                    ))(get_user_meta(today, uid, "gamble_multiplier", 1.0))
                ),
            },
            {
                "id": "multiple_personalities",
                "label": "多重人格",
                "desc": "多重人格：每隔2小时将你所有的状态及道具卡重新随机",
                "item_name": "多重人格",
                "checker": flag_checker("multiple_personalities"),
            },
            {
                "id": "sensitive_skin",
                "label": "敏感肌",
            ...
            },
            {
                "id": "rival",
                "label": "情敌",
                "desc": "情敌：与他人互为情敌，抽老婆时有概率将对方的老婆抢过来",
                "item_name": "情敌",
                "checker": flag_checker("rival"),
                "desc_generator": lambda today, uid, gid, plugin=self: plugin._rival_desc_generator(today, uid, gid),
            },
        ]
        ...
        self.commands = {
            ...
            "认老婆": self.recognize_wife_challenge,
            "老婆排行榜": self.show_leaderboard,
        }
    ...
    async def show_leaderboard(self, event: AstrMessageEvent):
        ...
        today_scores = []
        cfg = load_group_config(gid)
        for uid, record in cfg.items():
            if str(gid) in record.get("groups", []):
                nick = record.get("nick", f"用户{uid}")
                score = self._calculate_user_score(today, gid, uid, cfg)
                if score > 0:
                    today_scores.append({"uid": uid, "nick": nick, "score": score})
        today_scores.sort(key=lambda x: x["score"], reverse=True)
        today_scores = today_scores[:10]
        ...
        try:
            ...
            snapshot_file = os.path.join(CONFIG_DIR, "yesterday_leaderboard.json")
            if os.path.exists(snapshot_file):
                with open(snapshot_file, "r", encoding="utf-8") as f:
                    snap_data = json.load(f)
                if snap_data.get("date") == yesterday:
                    yesterday_scores = snap_data.get("groups", {}).get(gid, [])
        except Exception:
            pass
            
        if not today_scores and not yesterday_scores:
            yield event.plain_result(f"{event.get_sender_name()}，当前群内还没有人上榜哦，快去抽老婆、积攒道具吧！")
            return
            
        # 3. 生成排行榜图片
        try:
            img = self._generate_leaderboard_image(today_scores, yesterday_scores)
            temp_path = os.path.join(PLUGIN_DIR, f"leaderboard_{gid}_{today}.png")
            img.save(temp_path)
            ...
            if today_scores:
                lines.append("=== 今日榜 ===")
                for i, entry in enumerate(today_scores, 1):
                    lines.append(f"第{i}名：{entry['nick']} ({entry['score']}分)")
            if yesterday_scores:
                lines.append("=== 昨日榜 ===")
                for i, entry in enumerate(yesterday_scores, 1):
                    lines.append(f"第{i}名：{entry['nick']} ({entry['score']}分)")
            yield event.plain_result("\n".join(lines))

    def _generate_leaderboard_image(self, today_scores: list, yesterday_scores: list) -> PILImage.Image:
        width = 800
        padding = 40
        line_height = 40
        title_height = 80
        ...
        try:
            ...
            bold_font = ImageFont.truetype("msyhbd.ttc", 24)
        except:
            try:
        ...
        total_lines = 0
        if today_scores:
            total_lines += len(today_scores)
        else:
            total_lines += 1
            
        if yesterday_scores:
            total_lines += len(yesterday_scores)
        else:
            total_lines += 1
            
        header_row_height = 40
        height = padding * 2 + title_height + (subtitle_height + header_row_height) * 2 + total_lines * line_height + section_gap
        
        img = PILImage.new('RGB', (width, height), color=(250, 250, 255))
        ...
        draw.text(((width - title_w) // 2, current_y), title_text, fill=(20, 20, 20), font=title_font)
        ...
        def draw_table(scores, y_start, subtitle):
            y = y_start
            draw.text((padding, y), subtitle, fill=(70, 130, 180), font=subtitle_font)
            y += subtitle_height
            
            # 表头
            draw.rectangle([padding, y, width - padding, y + header_row_height], fill=(230, 240, 250))
            draw.text((padding + 20, y + 8), "排名", fill=(100, 100, 100), font=text_font)
            draw.text((padding + 150, y + 8), "用户名", fill=(100, 100, 100), font=text_font)
            draw.text((width - padding - 150, y + 8), "分数", fill=(100, 100, 100), font=text_font)
            y += header_row_height
            
            if not scores:
                draw.text((padding + 20, y + 8), "虚位以待", fill=(150, 150, 150), font=text_font)
                y += line_height
                return y
                
            for i, entry in enumerate(scores, 1):
                color = colors.get(i, default_color)
                font = bold_font if i <= 3 else text_font
                
                # 排名背景
                if i <= 3:
                    draw.ellipse([padding + 15, y + 5, padding + 45, y + 35], fill=color)
                    draw.text((padding + 24, y + 8), str(i), fill=(255, 255, 255), font=font)
                else:
                    draw.text((padding + 20, y + 8), str(i), fill=color, font=font)
                
                # 用户名，截断处理
                nick = entry['nick']
                max_nick_w = width - padding * 2 - 300
                while nick and (draw.textbbox((0, 0), nick, font=font)[2] - draw.textbbox((0, 0), nick, font=font)[0] > max_nick_w):
                    nick = nick[:-1]
                if nick != entry['nick']:
                    nick += "..."
                    
                draw.text((padding + 150, y + 8), nick, fill=color, font=font)
                draw.text((width - padding - 150, y + 8), f"{entry['score']}", fill=color, font=font)
                y += line_height
            return y
            
        current_y = draw_table(today_scores, current_y, "今日榜")
        current_y += section_gap
        current_y = draw_table(yesterday_scores, current_y, "昨日榜")
    ...
    def parse_at_target(self, event, ignore_zero_attention: bool = False, ignore_riddler: bool = False, riddler_messages: list = None):
        # 解析@目标用户
        # ignore_zero_attention: 是否忽略"0人问你"状态（管理员指令使用）
        # riddler_messages: 可选的列表，用于收集谜语人随机化的告知信息
        today = get_today()
        ...
        if original_target and not ignore_riddler:
            ...
            user_has_riddler = get_user_flag(today, uid, "riddler")
            target_has_riddler = get_user_flag(today, original_target, "riddler")
            # 检查使用者是否有谜语人状态
            if user_has_riddler:
                should_randomize = True
            # 检查目标是否有谜语人状态
            elif target_has_riddler:
                should_randomize = True
            
            if should_randomize:
                # 如果目标有谜语人状态，不排除任何人（包括原目标和使用者）
                # 如果只有使用者有谜语人状态，排除使用者（避免随机到自己）
                if target_has_riddler:
                    exclude_uid = None  # 不排除任何人
                else:
                    exclude_uid = uid  # 只排除使用者
                valid_targets = self._get_valid_targets_for_riddler(gid, exclude_uid=exclude_uid)
                if valid_targets:
                    random_target = random.choice(valid_targets)
                    # 如果提供了 riddler_messages 列表，添加告知信息
                    if riddler_messages is not None:
                        cfg = load_group_config(gid)
                        original_record = cfg.get(original_target, {}) if isinstance(cfg.get(original_target), dict) else {}
                        random_record = cfg.get(random_target, {}) if isinstance(cfg.get(random_target), dict) else {}
                        original_nick = original_record.get("nick", f"用户{original_target}")
                        random_nick = random_record.get("nick", f"用户{random_target}")
                        riddler_messages.append(f"【谜语人】目标已随机化：{original_nick} → {random_nick}")
    ...
    def parse_target(self, event, riddler_messages: list = None):
        # 解析命令目标用户
        target = self.parse_at_target(event, riddler_messages=riddler_messages)
        if target:
            return target
    ...
    def parse_multi_targets(
        ...
        if should_randomize and original_targets:
            # 检查是否有任何目标有谜语人状态
            any_target_has_riddler = any(get_user_flag(today, target_uid, "riddler") for target_uid in original_targets)
            # 如果任何目标有谜语人状态，不排除任何人（包括原目标和使用者）
            # 如果只有使用者有谜语人状态，排除使用者（避免随机到自己）
            if any_target_has_riddler:
                exclude_uid = None  # 不排除任何人
            else:
                exclude_uid = uid  # 只排除使用者
            valid_targets = self._get_valid_targets_for_riddler(gid, exclude_uid=exclude_uid)
            if valid_targets:
                ...
                while len(randomized_targets) < len(original_targets) and valid_targets:
                    random_target = random.choice(valid_targets)
                    randomized_targets.append(random_target)
                    # 如果提供了 riddler_messages 列表，添加告知信息（对于重复的目标）
                    if riddler_messages is not None:
                        random_record = cfg.get(random_target, {}) if isinstance(cfg.get(random_target), dict) else {}
                        random_nick = random_record.get("nick", f"用户{random_target}")
                        riddler_messages.append(f"【谜语人】目标已随机化：→ {random_nick}")
    ...
    def _format_flag_names(self, flags: set[str]) -> str:
        if not flags:
            return ""
        # 特殊处理：force_swap 是"公交车"状态的一部分，应该显示为"公交车"
        flag_label_mapping = {
            "force_swap": "公交车",
        }
        labels = []
        for flag in sorted(flags):
            # 先检查特殊映射
            if flag in flag_label_mapping:
                labels.append(flag_label_mapping[flag])
            else:
                # 然后检查 flag_label_map
                label = self.flag_label_map.get(flag, flag)
                labels.append(label)
        return "、".join(labels)
    ...
    def _handle_wife_loss(self, today: str, uid: str, loss_count: int = 1, gid: str = None, lost_wives_list: list[str] = None):
        ...
        # 记录失去的老婆到lost_wives（用于龙王道具卡）
        if lost_wives_list:
            lost_wives = get_user_meta(today, uid, "lost_wives", [])
            if not isinstance(lost_wives, list):
                lost_wives = []
            for wife in lost_wives_list:
                if isinstance(wife, str) and wife not in lost_wives:
                    lost_wives.append(wife)
            set_user_meta(today, uid, "lost_wives", lost_wives)
    ...
    def _list_wife_images(self) -> list[str]:
        try:
            return [
                f
                for f in os.listdir(IMG_DIR)
    ...
    def _answer_to_pinyin(self, text: str) -> str:
        if not text or lazy_pinyin is None:
            return ""
        normalized = unicodedata.normalize("NFKC", text)
        filtered = re.sub(r"[^a-zA-Z\u4e00-\u9fff]", "", normalized)
        if not filtered:
            return ""
    ...
    def _extract_wife_name_info(self, img: str) -> dict:
        ...
        if "!" in base:
            ...
            if chara:
                display_name = chara
            source_name = base.split("!", 1)[0].strip()
        alias_candidates = {display_name.strip(), base.strip()}
        separators = ["·", "・", "•", "、", "/", "\\", "-", "_", " ", "&", "+", "×"]
        expanded = set(alias_candidates)
        for alias in list(alias_candidates):
            for sep in separators:
                if sep in alias:
                    parts = [p.strip() for p in alias.split(sep) if p.strip()]
                    expanded.update(parts)
        expanded = {item for item in expanded if item}
        normalized_answers = {self._normalize_answer_text(alias) for alias in expanded}
        normalized_answers = {ans for ans in normalized_answers if ans}
        pinyin_variants = set()
        for alias in expanded:
            pinyin_text = self._answer_to_pinyin(alias)
            if pinyin_text:
                pinyin_variants.add(pinyin_text)
        return {
            "image": img,
            "display_name": display_name or base or "未知角色",
            "normalized_answers": normalized_answers,
            "pinyin_answers": pinyin_variants,
    ...
    def _prepare_recognize_questions(self, count: int = 3) -> list[dict]:
        ...
        questions = []
        for img in images:
            info = self._extract_wife_name_info(img)
            if not info.get("normalized_answers"):
                continue
    ...
    def _grant_random_items(
        ...
        *,
        ...
        if quality is not None:
            pool = [item for item in self.item_pool if self.get_item_quality(item) == quality]
        if not pool:
            return []
    ...
    def _grant_random_command_bonus(self, today: str, uid: str, base_amount: int, select_amount: int):
        command_pool = [
            {"label": "换老婆", "mod": "change_extra_uses"},
            {"label": "牛老婆", "mod": "ntr_extra_uses"},
            {"label": "选老婆", "mod": "select_wife_uses"},
            {"label": "打老婆", "mod": "beat_wife_uses"},
            {"label": "勾引", "mod": "seduce_uses"},
            {"label": "重置牛/换", "mod": "reset_extra_uses"},
            {"label": "重置盲盒", "mod": "reset_blind_box_extra"},
        ]
        selectable = list(command_pool)
        chosen = None
        while selectable:
            candidate = random.choice(selectable)
            if candidate["mod"] == "seduce_uses":
                current = int(get_user_mod(today, uid, "seduce_uses", 0))
                if current == -1:
                    selectable.remove(candidate)
                    continue
            chosen = candidate
            break
        if not chosen:
            chosen = command_pool[0]
        is_select = chosen["mod"] == "select_wife_uses"
        amount = select_amount if is_select else base_amount
        if chosen["mod"] == "seduce_uses":
            add_user_mod(today, uid, "seduce_uses", amount)
        else:
            add_user_mod(today, uid, chosen["mod"], amount)
        return chosen["label"], amount

    def _schedule_recognize_timeout(self, event: AstrMessageEvent, session_key: tuple[str, str], question_index: int, question: dict):
    ...
    async def _send_recognize_question_direct(self, event: AstrMessageEvent, session_key: tuple[str, str], session: dict):
        idx = session.get("current_index", 0)
        total = len(session.get("questions", [])) or 1
        question = session["questions"][idx]
        prompt = f"第{idx + 1}/{total}题：这位老婆是谁？请回复名字~"
        components = [Plain(prompt)]
        image_component = self._build_image_component(question["image"])
        if image_component:
    ...
    async def _handle_dingdong_effect(self, today: str, uid: str, target_uids: str | list[str], gid: str) -> str:
        """
        处理叮咚鸡/阳性传播效果，并返回传播的描述消息。
        """
        ...
        # 查找状态的中文显示名字
        state_label = chosen_state
        for spec in self.status_effect_specs:
            if spec.get("id") == chosen_state:
                state_label = spec.get("label", chosen_state)
                break
        ...
        for target_uid in target_uids:
            if target_uid and target_uid != uid:
                ...
                t_info = cfg.get(target_uid, {})
                t_nick = t_info.get("nick", f"用户{target_uid}") if isinstance(t_info, dict) else f"用户{target_uid}"
                infect_nicks.append(t_nick)

        msg_parts = []
        if infect_nicks:
            msg_parts.append(f"【叮咚鸡传染】你将身上的「{state_label}」状态传染给了：{', '.join(infect_nicks)}！")
        ...
        if has_dingdong:
            ...
            if random.random() < prob:
                ...
                msg_parts.append("【叮咚鸡突变】因为频繁传播状态，你的叮咚鸡发生了变异，你失去了「叮咚鸡」状态，并获得了「阳性」状态！")
    ...
    async def _redistribute_wives(self, gid: str, today: str, event: AstrMessageEvent, cfg: dict):
        ...
        for u, old_wives in today_users:
            if idx < len(all_images):
                ...
                rec["harem"] = False
                idx += 1
                self._handle_wife_loss(today, u, len(old_wives), gid, lost_wives_list=list(old_wives) if old_wives else [])
        for u, old_wives in harem_users.items():
            ...
            if idx + count <= len(all_images):
                ...
                idx += count
                self._handle_wife_loss(today, u, count, gid, lost_wives_list=list(old_wives) if old_wives else [])
        save_group_config(cfg)
    ...
    def _calculate_user_score(self, date: str, gid: str, uid: str, cfg: dict) -> int:
        score = 0
        
        # 1. Wives: each wife is 3 points
        # get_wife_count checks if the record date matches the given date
        wife_count = get_wife_count(cfg, uid, date)
        score += wife_count * 3
        
        # 2. Items: 1~5 points based on star rating
        date_items = item_data.get(date, {})
        user_items = date_items.get(uid, [])
        for item in user_items:
            quality = self.item_quality.get(item, 1)
            score += quality
            
        # 3. Statuses: 1~5 points based on star rating
        day_effects = effects_data.get(date, {})
        user_eff = day_effects.get(uid, {})
        flags = user_eff.get("flags", {})
        
        for flag_id, is_active in flags.items():
            if is_active:
                spec = next((s for s in self.status_effect_specs if s.get("id") == flag_id), None)
                if spec:
                    item_name = spec.get("item_name")
                    if item_name:
                        quality = self.item_quality.get(item_name, 1)
                        score += quality
                    else:
                        score += 2 # 默认状态分数
        
        # 4. 成功牛一次老婆加 1 分
        ntr_rec = ntr_records.get(uid)
        if ntr_rec and ntr_rec.get("date") == date:
            score += ntr_rec.get("count", 0) * 1
            
        # 5. 成功赠送一次道具加 5 分
        gift_rec = gift_records.get(uid)
        if gift_rec and gift_rec.get("date") == date:
            score += gift_rec.get("count", 0) * 5
            
        return score

    def _generate_leaderboard_snapshot(self, date: str, snapshot_file: str):
        snapshot = {"date": date, "groups": {}}
        # 遍历所有存在的群
        # 注意: _group_config_cache 可能不包含所有群，所以我们遍历 wives_data 收集群ID
        all_gids = set()
        for uid, record in wives_data.items():
        ...
        for gid in all_gids:
            ...
            for uid, record in cfg.items():
                if str(gid) in record.get("groups", []):
                    nick = record.get("nick", f"用户{uid}")
                    score = self._calculate_user_score(date, gid, uid, cfg)
                    if score > 0:
                        scores.append({"uid": uid, "nick": nick, "score": score})
            # 按分数降序，取前10名
            scores.sort(key=lambda x: x["score"], reverse=True)
            snapshot["groups"][gid] = scores[:10]
    ...
    async def on_all_messages(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if should_snapshot:
                self._generate_leaderboard_snapshot(yesterday, snapshot_file)
        except Exception as e:
            print(f"Error checking leaderboard snapshot: {e}")
        ...
        if eff["flags"].get("multiple_personalities"):
            meta = eff.setdefault("meta", {})
            mp_expire_ts = meta.get("mp_expire_ts")
            now_ts = datetime.utcnow().timestamp()
            if not mp_expire_ts:
                meta["mp_expire_ts"] = now_ts + 7200
                save_effects()
            elif now_ts >= mp_expire_ts:
                meta["mp_expire_ts"] = now_ts + 7200
                ...
                user_items = today_items.get(uid, [])
                if user_items:
                    lost_items_count = len(user_items)
                    user_items.clear()
                    for _ in range(lost_items_count):
                        user_items.append(random.choice(self.item_pool))
                    save_item_data()
                
                # Randomize statuses (keep multiple_personalities)
                flags = eff.get("flags", {})
                lost_states_count = sum(1 for k, v in flags.items() if v and k != "multiple_personalities")
                for k in flags:
    ...
    async def recognize_wife_challenge(self, event: AstrMessageEvent):
        gid = str(event.message_obj.group_id)
        uid = str(event.get_sender_id())
        today = get_today()
        ...
        if session_key in self.recognize_sessions:
            yield event.plain_result("认老婆挑战正在进行中，请先回答当前题目哦~")
            return
        questions = self._prepare_recognize_questions(3)
        if len(questions) < 3:
            yield event.plain_result("当前老婆素材不足，暂时无法开启认老婆挑战，请稍后再试~")
            return
        session = {
            "today": today,
            "gid": gid,
    ...
    async def _send_recognize_question(self, event: AstrMessageEvent, session_key: tuple[str, str], session: dict):
        idx = session.get("current_index", 0)
        total = len(session.get("questions", [])) or 1
        question = session["questions"][idx]
        prompt = f"第{idx + 1}/{total}题：这位老婆是谁？请回复名字~"
    ...
    async def _handle_recognize_answer(self, event: AstrMessageEvent, session_key: tuple[str, str]):
        ...
        if session.get("today") != today:
            self.recognize_sessions.pop(session_key, None)
            yield event.plain_result("认老婆挑战已过期，请重新发送「认老婆」开始新的挑战。")
            return
        self_id = getattr(event.message_obj, "self_id", None)
        if self_id is not None and not self.is_at_me(event.message_obj.message, self_id):
            session["no_at_count"] = session.get("no_at_count", 0) + 1
            if session["no_at_count"] >= 3:
                yield event.plain_result("请@我并附上答案，例如：@我 角色名")
                session["no_at_count"] = 0
            return
        session["no_at_count"] = 0
        idx = session.get("current_index", 0)
        ...
        session["awaiting_answer"] = False
        self._cancel_recognize_timeout(session)
        is_correct = self._is_recognize_answer_correct(question, answer_text)
        if is_correct:
            session["correct"] += 1
        verdict = "答对啦！" if is_correct else "可惜答错了~"
        source_name = question.get("source_name", "")
        if source_name:
            answer_text = f"来自《{source_name}》的{question['display_name']}"
        else:
            answer_text = question["display_name"]
        yield event.plain_result(f"{event.get_sender_name()}，{verdict} 正确答案：{answer_text}")
        session["current_index"] += 1
    ...
    def _finalize_recognize_rewards(
        ...
        if correct_count == 1:
            ...
            if reward_type == "items":
                random_items = self._grant_random_items(today, uid, 3)
                reward_text = f"随机道具卡×3：{'、'.join(random_items)}" if random_items else "尝试发放随机道具卡但卡池为空。"
            elif reward_type == "commands":
                cmd_label, cmd_amount = self._grant_random_command_bonus(today, uid, 3, 1)
                reward_text = f"随机指令「{cmd_label}」次数 +{cmd_amount}"
            else:
        ...
        if correct_count == 2:
            ...
            if reward_type == "items":
                random_items = self._grant_random_items(today, uid, 5)
                reward_text = f"随机道具卡×5：{'、'.join(random_items)}" if random_items else "尝试发放随机道具卡但卡池为空。"
            elif reward_type == "commands":
                cmd_label, cmd_amount = self._grant_random_command_bonus(today, uid, 5, 2)
                reward_text = f"随机指令「{cmd_label}」次数 +{cmd_amount}"
            elif reward_type == "draw_reset":
                add_user_mod(today, uid, "blind_box_extra_draw", 1)
                add_user_mod(today, uid, "reset_blind_box_extra", 1)
                reward_text = "抽盲盒额外机会 +1，并获得重置盲盒额外机会 +1"
            else:
        ...
        if reward_type == "items":
            random_items = self._grant_random_items(today, uid, 7)
            reward_text = f"随机道具卡×7：{'、'.join(random_items)}" if random_items else "尝试发放随机道具卡但卡池为空。"
        elif reward_type == "commands":
            cmd_label, cmd_amount = self._grant_random_command_bonus(today, uid, 7, 3)
            reward_text = f"随机指令「{cmd_label}」次数 +{cmd_amount}"
        elif reward_type == "draw_reset":
            add_user_mod(today, uid, "blind_box_extra_draw", 2)
            add_user_mod(today, uid, "reset_blind_box_extra", 1)
            reward_text = "抽盲盒额外机会 +2，并获得重置盲盒额外机会 +1"
        elif reward_type == "legend":
            legend_items = self._grant_random_items(today, uid, 5, quality=5)
            reward_text = f"传说道具卡×5：{'、'.join(legend_items)}" if legend_items else "尝试发放传说道具卡但卡池为空。"
        else:
            perk_type = random.choice(["item_count", "crit_rate", "empty"])
            if perk_type == "item_count":
                new_value = add_blind_box_perk(uid, "item_count_bonus", 1, max_value=5)
                reward_text = f"抽盲盒永久加成：掉落数量上限 +1（当前+{new_value}）"
            elif perk_type == "crit_rate":
                new_value = add_blind_box_perk(uid, "crit_rate_bonus", 0.02, max_value=0.10)
                reward_text = f"抽盲盒永久加成：暴击率 +2%（当前+{int(new_value * 100)}%）"
            else:
    ...
    async def draw_item(self, event: AstrMessageEvent):
        ...
        if get_user_flag(today, uid, "maximize_use"):
            if user_items and len(user_items) > 0:
                yield event.plain_result(f"{event.get_sender_name()}，光盘行动：你还有未使用的道具卡，请先使用完所有道具卡后再抽盲盒哦~")
                return
        
        # 检查盲盒爱好者状态：每2小时可以免费抽一次盲盒
        ...
        if is_blind_box_enthusiast:
            ...
            two_hours_ago = now - 7200  # 2小时 = 7200秒
            
            if last_free_draw > two_hours_ago:
                # 距离上次免费抽盲盒不足2小时
                wait_seconds = int(7200 - (now - last_free_draw))
                ...
                yield event.plain_result(f"{event.get_sender_name()}，你作为盲盒爱好者，每2小时可以免费抽一次盲盒，请等待{wait_text}后再试~")
                return
            ...
            if len(blind_box_groups) > 0 and not allow_extra_draw:
                yield event.plain_result(f"{event.get_sender_name()}，你今天已经抽过盲盒啦，明天再来吧~")
                return
        ...
        has_guaranteed = get_user_flag(today, uid, "guaranteed_pool")
        ...
        while pending_batches > 0:
            ...
            if is_crit_batch and batch_count > 0:
                ...
                for item in quality_pools[5]:
                    ...
                    if item in self.status_items and item in drawn_status:
                        continue
                    if has_guaranteed and (item in existing_items or item in drawn_items):
                        continue
            ...
            # 盲盒基础暴击率减少到10%
            base_crit_rate = 0.10 + crit_rate_bonus
            if get_user_flag(today, uid, "blind_box_enthusiast"):
                base_crit_rate = base_crit_rate * 0.50
            # 连续暴击几率递减：每次暴击后都会减少当次抽盲盒的暴击率的20%
            base_crit_rate = base_crit_rate * (0.8 ** crit_times)
    ...
    async def reset_blind_box(self, event: AstrMessageEvent):
        ...
        if get_user_flag(today, uid, "lightbulb"):
            yield event.plain_result(f"{event.get_sender_name()}，电灯泡状态下无法使用「重置盲盒」指令哦~")
            return
        # 检查光盘行动状态：在有未使用的道具卡的情况下无法重置盲盒
        if get_user_flag(today, uid, "maximize_use"):
            user_items = today_items.get(uid)
            if user_items and len(user_items) > 0:
                yield event.plain_result(f"{event.get_sender_name()}，光盘行动：你还有未使用的道具卡，请先使用完所有道具卡后再重置盲盒哦~")
                return
        # 自己重置
        if not arg:
            ...
            if used >= max_reset_blind_box:
                yield event.plain_result(f"{event.get_sender_name()}，你今天已经使用过「重置盲盒」{max_reset_blind_box}次啦~")
                return
            ...
            yield event.plain_result(f"{event.get_sender_name()}，你的盲盒次数已重置，当前道具已清空，可以重新抽取啦！")
            return
        # 重置指定目标
        if arg == "所有人":
            if uid not in admin_ids:
                yield event.plain_result(f"{event.get_sender_name()}，仅管理员才能重置所有人的盲盒次数哦~")
                return
            affected = 0
            for target_uid in list(today_items.keys()):
                ...
                if gid in valid_groups:
                    ...
                    affected += 1
            if affected == 0:
                yield event.plain_result(f"{event.get_sender_name()}，今天还没有需要重置盲盒的群成员哦~")
            else:
                yield event.plain_result(f"{event.get_sender_name()}，已为本群{affected}位成员重置盲盒次数，已保留他们现有的道具卡。")
            return
        is_admin = uid in admin_ids
        target_uid = self.parse_at_target(event, ignore_riddler=is_admin)
        if not target_uid:
            yield event.plain_result(f"{event.get_sender_name()}，请在“重置盲盒”后@需要重置的目标用户哦~")
            return
        if not is_admin:
            yield event.plain_result(f"{event.get_sender_name()}，仅管理员才能为他人重置盲盒次数哦~")
            return
        target_uid = str(target_uid)
        if target_uid not in today_items:
            yield event.plain_result(f"{event.get_sender_name()}，对方今天还没有抽过盲盒，无需重置哦~")
            return
        ...
        yield event.plain_result(f"{event.get_sender_name()}，已为 {target_nick} 重置盲盒次数并清空其今日道具。")
    ...
    async def view_items(self, event: AstrMessageEvent):
        ...
        if user_items is None or len(user_items) == 0:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有道具卡，快去抽盲盒吧~")
            return
        items_text = "、".join(user_items)
        yield event.plain_result(f"{event.get_sender_name()}，你当前拥有的道具卡：{items_text}")
    ...
    async def gift_item(self, event: AstrMessageEvent):
        ...
        if rec["count"] >= max_gift_uses:
            yield event.plain_result(f"{event.get_sender_name()}，你今天已经发起了{max_gift_uses}次赠送请求，明天再来吧~")
            return
        riddler_messages = []
        target_uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        if not target_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n使用「赠送」时请@目标用户哦~" if riddler_info else f"使用「赠送」时请@目标用户哦~"
            yield event.plain_result(result_msg)
            return
        target_uid = str(target_uid)
        if target_uid == uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n不能把道具送给自己哦~" if riddler_info else f"不能把道具送给自己哦~"
            yield event.plain_result(result_msg)
            return
        text = event.message_str.strip()
        ...
        if not item_name:
            yield event.plain_result(f"{event.get_sender_name()}，请写明要赠送的道具卡名称哦~")
            return
        today_items = item_data.setdefault(today, {})
        user_items = today_items.get(uid, [])
        if item_name not in user_items:
            yield event.plain_result(f"{event.get_sender_name()}，你的道具卡里没有「{item_name}」哦~")
            return
        # 检查是否有"淳平"状态
        ...
        if has_junpei:
            ...
            if not user_has_equal_rights and not target_has_equal_rights:
                ...
                if get_user_flag(today, target_uid, "ban_items"):
                    yield event.plain_result(f"{event.get_sender_name()}，对方正处于贤者时间，无法接收道具。")
                    return
            # 从用户背包中移除道具
            ...
            if success and effect_msg:
                yield event.plain_result(f"{event.get_sender_name()}，已成功将「{item_name}」赠送给{target_nick}并立即使用！{effect_msg}{bonus_msg}")
            elif success:
                yield event.plain_result(f"{event.get_sender_name()}，已成功将「{item_name}」赠送给{target_nick}并立即使用！{bonus_msg}")
            else:
                yield event.plain_result(f"{event.get_sender_name()}，已成功将「{item_name}」赠送给{target_nick}，但使用失败：{effect_msg}{bonus_msg}")
        else:
            # 正常流程：需要对方同意
            # 增加赠送次数
            rec["count"] += 1
    ...
    async def accept_gift(self, event: AstrMessageEvent):
        ...
        nick = event.get_sender_name()
        riddler_messages = []
        sender_uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        if not sender_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n请在「同意赠送」后@发起赠送的人哦~" if riddler_info else f"请在「同意赠送」后@发起赠送的人哦~"
            yield event.plain_result(result_msg)
            return
        ...
        if not record:
            yield event.plain_result(f"{event.get_sender_name()}，当前没有来自该用户的赠送请求哦~")
            return
        ...
        if item_name not in sender_items:
            ...
            yield event.plain_result(f"{event.get_sender_name()}，对方已经没有「{item_name}」了，赠送请求失效啦~")
            return
        ...
        if not target_bucket:
            donations.pop(receiver_uid, None)
        self._cleanup_gift_entry(today, gid)
        yield event.plain_result(f"{event.get_sender_name()}，已成功领取{sender_nick}赠送的「{item_name}」！{bonus_msg}")
    ...
    async def reject_gift(self, event: AstrMessageEvent):
        ...
        if not sender_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n请在「拒绝赠送」后@发起赠送的人哦~" if riddler_info else f"请在「拒绝赠送」后@发起赠送的人哦~"
            yield event.plain_result(result_msg)
            return
        sender_uid = str(sender_uid)
        entry = self._get_gift_group_entry(today, gid)
        ...
        sender_info = cfg.get(sender_uid, {})
        sender_nick = sender_info.get("nick", f"用户{sender_uid}") if isinstance(sender_info, dict) else f"用户{sender_uid}"
        yield event.plain_result(f"{event.get_sender_name()}，已拒绝{sender_nick}赠送的「{item_name}」，请求已撤销。")
    ...
    async def request_item(self, event: AstrMessageEvent):
        ...
        if rec.get("date") != today:
            rec = {"date": today, "count": 0}
        if rec["count"] >= 3:
            yield event.plain_result(f"{event.get_sender_name()}，你今天已经发起了3次索取请求，明天再来吧~")
            return
        riddler_messages = []
        target_uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        if not target_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n使用「索取」时请@目标用户哦~" if riddler_info else f"使用「索取」时请@目标用户哦~"
            yield event.plain_result(result_msg)
            return
        target_uid = str(target_uid)
        if target_uid == uid:
            yield event.plain_result(f"{event.get_sender_name()}，不能向自己索取道具哦~")
            return
        text = event.message_str.strip()
        ...
        item_name = item_name.strip("【】「」『』\"'"" ")
        if not item_name:
            yield event.plain_result(f"{event.get_sender_name()}，请写明要索取的道具卡名称哦~")
            return
        today_items = item_data.setdefault(today, {})
        target_items = today_items.get(target_uid, [])
        if item_name not in target_items:
            yield event.plain_result(f"{event.get_sender_name()}，对方当前没有「{item_name}」，无法索取哦~")
            return
        # 增加索取次数
        rec["count"] += 1
        ...
        target_nick = target_info.get("nick", f"用户{target_uid}") if isinstance(target_info, dict) else f"用户{target_uid}"
        riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
        result_msg = f"{riddler_info}\n已向{target_nick}发起索取「{item_name}」的请求，请TA发送「同意索取 @{nick}」确认。" if riddler_info else f"已向{target_nick}发起索取「{item_name}」的请求，请TA发送「同意索取 @{nick}」确认。"
        yield event.plain_result(result_msg)
    ...
    async def accept_request(self, event: AstrMessageEvent):
        ...
        nick = event.get_sender_name()
        riddler_messages = []
        requester_uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        if not requester_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n请在「同意索取」后@请求道具的用户哦~" if riddler_info else f"请在「同意索取」后@请求道具的用户哦~"
            yield event.plain_result(result_msg)
            return
        ...
        if not record:
            yield event.plain_result(f"{event.get_sender_name()}，没有来自该用户的索取请求哦~")
            return
        item_name = record["item"]
        today_items = item_data.setdefault(today, {})
        ...
        if item_name not in giver_items:
            ...
            self._cleanup_gift_entry(today, gid)
            yield event.plain_result(f"{event.get_sender_name()}，你已经没有「{item_name}」了，此次索取请求自动失效~")
            return
        ...
        yield event.plain_result(f"{event.get_sender_name()}，已同意索取请求，「{item_name}」已交给{requester_nick}。{bonus_msg}")
    ...
    async def reject_gift(self, event: AstrMessageEvent):
        ...
        if not target_bucket:
            donations.pop(receiver_uid, None)
        self._cleanup_gift_entry(today, gid)
        yield event.plain_result(f"{event.get_sender_name()}，已拒绝{sender_nick}赠送的「{item_name}」请求。")
    ...
    async def reject_request(self, event: AstrMessageEvent):
        ...
        if not requester_uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n请在「拒绝索取」后@请求道具的用户哦~" if riddler_info else f"请在「拒绝索取」后@请求道具的用户哦~"
            yield event.plain_result(result_msg)
            return
        requester_uid = str(requester_uid)
        entry = self._get_gift_group_entry(today, gid)
        ...
        if not giver_bucket:
            demands.pop(giver_uid, None)
        self._cleanup_gift_entry(today, gid)
        yield event.plain_result(f"{event.get_sender_name()}，已拒绝{requester_nick}索取「{item_name}」的请求。")
    ...
    async def view_item_requests(self, event: AstrMessageEvent):
        ...
        if not lines:
            yield event.plain_result(f"{event.get_sender_name()}，目前没有待你处理的道具赠送或索取请求。")
            return
    ...
    async def view_status(self, event: AstrMessageEvent):
        ...
        # 盲盒永久加成
        item_count_bonus = get_blind_box_perk(uid, "item_count_bonus", 0)
        if item_count_bonus > 0:
            panel_data.append(f"盲盒永久加成：道具数量+{item_count_bonus}")
        crit_rate_bonus = get_blind_box_perk(uid, "crit_rate_bonus", 0)
        if crit_rate_bonus > 0:
            panel_data.append(f"盲盒永久加成：暴击率+{int(crit_rate_bonus * 100)}%")
        empty_reduction_bonus = get_blind_box_perk(uid, "empty_reduction_bonus", 0)
        if empty_reduction_bonus > 0:
            panel_data.append(f"盲盒永久加成：空抽概率-{int(empty_reduction_bonus * 100)}%")
        ...
        if riddler_messages:
            ...
            yield event.plain_result(riddler_info)
        
        # 如果没有数据，返回文字提示
        if not panel_data and not status_data:
            if target_uid:
                yield event.plain_result(f"{event.get_sender_name()}，{nick}目前没有任何状态效果，安心抽老婆吧~")
            else:
                yield event.plain_result(f"{event.get_sender_name()}，你目前没有任何状态效果，安心抽老婆吧~")
            return
        ...
        sections = [
            (
                ...
                [
                    ...
                    "老婆排行榜：查看今日和昨日的双榜单，按老婆数(+3)、道具卡与状态稀有度(+1~5)、牛老婆(+1)及赠送道具(+5)计分",
                    "老婆插件帮助：查看本说明",
                ],
            ),
            (
    ...
    async def use_item(self, event: AstrMessageEvent):
        ...
        if get_user_flag(today, uid, "ban_items"):
            yield event.plain_result(f"{event.get_sender_name()}，你正处于「贤者时间」，2小时内无法使用任何道具卡哦~")
            return
        
        # 检查盲盒爱好者状态：每3分钟只能使用一个道具
        is_blind_box_enthusiast = get_user_flag(today, uid, "blind_box_enthusiast")
        if is_blind_box_enthusiast:
            now = datetime.utcnow().timestamp()
            last_item_use = get_user_meta(today, uid, "last_item_use_time", 0)
            three_minutes_ago = now - 180  # 3分钟 = 180秒
            
            if last_item_use > three_minutes_ago:
                # 距离上次使用道具不足3分钟
                wait_seconds = int(180 - (now - last_item_use))
                ...
                yield event.plain_result(f"{event.get_sender_name()}，你作为盲盒爱好者，每3分钟只能使用一个道具，请等待{wait_text}后再试~")
                return
        text = event.message_str.strip()
        content = text[len("使用") :].strip()
        if not content:
            yield event.plain_result(f"{event.get_sender_name()}，请在“使用”后跟上道具名称哦~")
            return
        parts = re.split(r"\s+|@", content, maxsplit=1)
        card_name = parts[0] if parts else ""
        if not card_name:
            yield event.plain_result(f"{event.get_sender_name()}，请明确要使用的道具名称哦~")
            return
        if card_name not in self.item_pool:
            yield event.plain_result(f"{event.get_sender_name()}，暂未识别到名为“{card_name}”的道具卡~")
            return
        extra_arg = content[len(card_name) :].strip()
        riddler_messages = []
        target_uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        if card_name in self.items_need_target and not target_uid:
            yield event.plain_result(f"{event.get_sender_name()}，使用「{card_name}」时请@目标哦~")
            return
        today_items = item_data.setdefault(today, {})
        user_items = today_items.setdefault(uid, [])
        if not user_items:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有抽盲盒，暂时没有可用的道具卡~")
            return
        ...
        with data_manager.transaction("item_data", "effects_data", "wives_data"):
            ...
            if target_uid:
                ...
                counter_msg = await self._trigger_sensitive_skin(event, today, gid, uid, str(target_uid))
                if counter_msg:
                    message = f"{message}\n{counter_msg}" if message else counter_msg
            # 如果有谜语人随机化信息，添加到消息中
            if riddler_messages:
                riddler_info = "\n".join(riddler_messages)
                message = f"{riddler_info}\n{message}" if message else riddler_info
        if success:
            if card_name == "洗牌":
                ...
                if bonus_card:
                    if bonus_card.endswith("[FAKE]"):
                        extra_msg = "魔术回路发动！你获得了赝品..."
                    else:
                    ...
                    # 纯爱战士效果：每次使用道具有10%概率获得白月光道具卡
                    if get_user_flag(today, uid, "protect_from_ntr") and self._probability_check(0.1, today, uid, positive=True):
                        if "白月光" in self.item_pool:
                            user_items.append("白月光")
                            save_item_data()
                            pure_love_msg = f"纯爱战士的守护！你获得了「白月光」道具卡。"
                            message = f"{message}\n{pure_love_msg}" if message else pure_love_msg
        ...
        *,
        ...
        async def finalize(success_flag: bool, message: str | None, *, double_effect_used: bool = False):
            ...
            if success_flag and target_uid:
                ...
                dingdong_msg = await self._handle_dingdong_effect(today, uid, str(target_uid), gid)
                if dingdong_msg and message:
                    message = f"{message}\n{dingdong_msg}"
                elif dingdong_msg:
                    message = dingdong_msg
            # 阳性状态：每次使用道具成功后，有 35% 的概率（负面概率，受运势等影响）随机失去一个状态（包括阳性状态本身）
            if success_flag and get_user_flag(today, uid, "positive"):
                # 概率触发损耗（基础35%，受运势调整）
                loss_trigger_prob = self._calculate_probability(
                    0.35, today, uid,
                    gain_multiplier=1.0,
                    apply_special=True,
                    apply_final=True,
                ...
                if random.random() < loss_trigger_prob:
                    eff_pos = get_user_effects(today, uid)
                    flags_pos = eff_pos.get("flags", {})
                    # 收集当前所有已激活状态（带 item_name 的，以及阳性状态本身）
                    ...
                    for spec in self.status_effect_specs:
                        ...
                        # 阳性状态本身也可以被随机失去（判定为3星品质）
                        if sid == "positive":
                            candidates_all.append((sid, 3))
                            continue
                    ...
                    if candidates_all:
                        ...
                        if pool:
                            ...
                            lose_label = lose_sid
                            for spec in self.status_effect_specs:
                                if spec.get("id") == lose_sid:
                                    lose_label = spec.get("label", lose_sid)
                                    break
                                    
                            if lose_sid == "positive":
                                positive_msg = f"【阳性痊愈】因为你处于「阳性」状态，你使用道具时有概率失去了一个状态。你非常幸运地失去了「阳性」状态，你康复了！"
                            else:
                                positive_msg = f"【阳性损耗】因为你处于「阳性」状态，你使用道具时有概率失去了一个状态：你随机失去了状态「{lose_label}」！"
                            if message:
                                message = f"{message}\n{positive_msg}"
                            else:
        ...
        if name == "牛魔王":
            ...
            if base_total > 0:
                add_user_mod(today, uid, "ntr_extra_uses", base_total * double_factor)
            save_effects()
        ...
        if name == "开impart":
            ...
            if success:
                ...
                if affected_targets:
                    ...
                    dingdong_msg = await self._handle_dingdong_effect(today, uid, affected_targets, gid)
                    if dingdong_msg and message:
                        message = f"{message}\n{dingdong_msg}"
                    elif dingdong_msg:
                        message = dingdong_msg
        ...
        if name == "转生":
            ...
            for key in list(flags.keys()):
                flags[key] = False
            save_effects()
            # 每失去1个状态，获得随机道具卡（数量受二度寝翻倍）
            gained_count = lost_count * double_factor
            gained = self._grant_random_items(today, uid, gained_count)
            if gained:
                gained_text = "、".join(gained)
                msg = f"你已重获新生！你失去了全部 {lost_count} 个状态，并获得了 {gained_count} 张随机道具卡：{gained_text}。"
            else:
        ...
        if name == "谁问你了":
            ...
            # 持续时间受二度寝翻倍（原为3小时，即10800秒）
            duration = 10800 * double_factor
            eff = get_user_effects(today, uid)
            eff["meta"]["zero_attention_expire_ts"] = datetime.utcnow().timestamp() + duration
            save_effects()
            hours = int(duration / 3600)
            return await finalize(True, f"0人问你状态生效！{hours}小时内别人@不到你，悠闲摸鱼去吧。", double_effect_used=double_active and double_factor > 1)
        if name == "接盘侠":
        ...
        if name == "月老":
            ...
            if len(targets) < 2:
                ...
                return await finalize(False, result_msg)
            a_uid, b_uid = targets[0], targets[1]
            if a_uid == b_uid:
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n月老需要@两个不同的目标，请重新选择。" if riddler_info else f"月老需要@两个不同的目标，请重新选择。"
                return await finalize(False, result_msg)
            cfg = load_group_config(gid)
        ...
        if name == "最后的波纹":
            ...
            # 收集所有活跃的flags（排除一些不应该转移的状态和辅助状态）
            exclude_flags = {"ban_items", "ban_reject_swap"}  # 贤者时间不转移，禁止拒绝交换作为苦主附带状态不单独转移
            ...
            for flag_key in selected_flags:
                ...
                # 特殊处理：如果是转移苦主状态，则需要同时转移其附带的禁止拒绝交换标志
                if flag_key == "victim_auto_ntr":
                    set_user_flag(today, uid, "ban_reject_swap", False, propagate=False)
                    set_user_flag(today, target_uid, "ban_reject_swap", True, propagate=False)
        ...
        if name == "富哥":
            ...
            if affected_targets:
                ...
                dingdong_msg = await self._handle_dingdong_effect(today, uid, affected_targets, gid)
                if dingdong_msg:
                    cupid_msg = f"{cupid_msg}\n{dingdong_msg}" if cupid_msg else dingdong_msg
            msg = f"你成为了富哥，今日额外获得{2 * double_factor}次老婆集市购买机会{target_nick_info}。"
            if cupid_msg:
                msg = f"{msg}\n{cupid_msg}"
            return await finalize(True, msg, double_effect_used=double_active and double_factor > 1)
        if name == "疯狂星期四":
            # 检查今天是否是星期四（weekday()返回0-6，0是周一，3是周四）
            is_thursday = datetime.utcnow().weekday() == 3
            
            if is_thursday:
                # 星期四效果：获得4次额外购买老婆的次数
                thursday_bonus = 4 * double_factor
                add_user_mod(today, uid, "market_wife_extra_purchases", thursday_bonus)
                return await finalize(True, f"今天真的是疯狂星期四！你额外获得{thursday_bonus}次只能购买老婆的集市机会。", double_effect_used=double_active and double_factor > 1)
            else:
                # 非星期四效果：额外获得一次只能购买老婆的集市次数
                base_bonus = 1 * double_factor
                add_user_mod(today, uid, "market_wife_extra_purchases", base_bonus)
                return await finalize(True, f"疯狂星期四来啦！你额外获得{base_bonus}次只能购买老婆的集市机会。", double_effect_used=double_active and double_factor > 1)
        if name == "硬凹":
        ...
        if name == "苦命鸳鸯":
            ...
            # 先保存失去的老婆列表
            user_lost_wives = list(get_wives_list(cfg, uid, today)) if user_loss > 0 else []
            target_lost_wives = list(get_wives_list(cfg, target_uid, today)) if target_loss > 0 else []
            if uid in cfg:
                del cfg[uid]
            if target_uid in cfg:
                del cfg[target_uid]
            user_fortune_msg = self._handle_wife_loss(today, uid, user_loss, gid, lost_wives_list=user_lost_wives)
            target_fortune_msg = self._handle_wife_loss(today, target_uid, target_loss, gid, lost_wives_list=target_lost_wives)
        ...
        if name == "牛道具":
            ...
            steal_count = min(random.randint(0, 2) * double_factor, len(target_items))
            if steal_count == 0:
                return await finalize(True, f"{equal_rights_msg}你伸出黑手，但尴尬地牵到了对方的手，牛道具失败了......", double_effect_used=double_active and double_factor > 1)
            ...
            # 触发陷阱卡检查
            trap_msg = self._check_and_trigger_trap(today, target_uid, uid, stolen, gid)
            if trap_msg:
                msg += trap_msg
            if target_fortune_msg:
                msg += f"\n{target_nick}{target_fortune_msg.replace('祸福相依：', '的祸福相依：')}"
        ...
        if name == "偷拍":
            ...
            if uid in cfg:
                del cfg[uid]
            fortune_msg = self._handle_wife_loss(today, uid, user_loss, gid, lost_wives_list=user_lost_wives)
        ...
        if name == "儒夫":
            ...
            return await finalize(True, f"儒家思想已融入你的血液，你今天获得了{10 * double_factor}次「打老婆」的使用次数。", double_effect_used=double_active and double_factor > 1)
        ...
        if name == "宝刀未老":
            ...
            return await finalize(True, f"宝刀未老！你今天额外获得{bonus_uses}次「牛老婆」机会。", double_effect_used=double_active and double_factor > 1)
        if name == "龙王":
            lost_wives = get_user_meta(today, uid, "lost_wives", [])
            ...
            save_group_config(cfg)
            return await finalize(True, f"龙王降临！你开启了后宫模式，并取回了所有被牛走的老婆。", double_effect_used=double_active and double_factor > 1)
        if name == "鹿鹿时间到了":
        ...
        if name == "发牌":
            ...
            if not members:
                members = {uid}
            cards_per_user = max(1, int(round(double_factor)))
            today_items = item_data.setdefault(today, {})
            preview = []
        ...
        if name == "烧火棍":
            ...
            if uid in cfg:
                # 先保存失去的老婆列表
                lost_wives_list = list(get_wives_list(cfg, uid, today)) if loss > 0 else []
                del cfg[uid]
                save_group_config(cfg)
                wife_fortune_msg = self._handle_wife_loss(today, uid, loss, gid, lost_wives_list=lost_wives_list)
            # 清空牛老婆记录（如果日期是今天）
            ...
            if wife_fortune_msg:
                msg += f"\n{wife_fortune_msg}"
        if name == "空白卡":
            return await finalize(False, "「空白卡」无任何效果，且不可使用。")
        if name == "陷阱卡":
            return await finalize(False, "「陷阱卡」不可主动使用。当自己因其他玩家的操作失去了这张卡时会自动反击。")
        if name == "保配盲盒":
            # 获得保配状态
        ...
        if name == "情敌":
            ...
            target_uid = str(target_uid)
            if target_uid == uid:
                return await finalize(False, "不能对自己使用「情敌」哦~")
            ...
            if not user_has_equal_rights and not target_has_equal_rights:
                if get_user_flag(today, target_uid, "ban_items"):
                    return await finalize(False, f"对方正处于贤者时间，无法对其使用「情敌」。")
            else:
                if get_user_flag(today, target_uid, "ban_items"):
                    if user_has_equal_rights:
                        equal_rights_msg = "众生平等！你无视了对方的贤者时间，"
            ...
            # 自己与目标均获得情敌状态
            set_user_flag(today, uid, "rival", True)
            set_user_meta(today, uid, "rival_target", target_uid)
            
            set_user_flag(today, target_uid, "rival", True)
            set_user_meta(today, target_uid, "rival_target", uid)
            
            target_info = cfg.get(target_uid, {})
            target_nick = target_info.get("nick", f"用户{target_uid}") if isinstance(target_info, dict) else f"用户{target_uid}"
            
            return await finalize(True, f"{equal_rights_msg}你对 {target_nick} 使用了「情敌」卡！你与 {target_nick} 互为情敌，抽老婆时有概率将对方的老婆抢过来！")
        ...
        if name == "报仇":
            ...
            if not interacting_users:
                return await finalize(False, "今天还没有人对你使用过指令或道具卡哦，无需报仇。")
            
            targets_count = len(interacting_users)
            lost_items_total = 0
            lost_wives_total = 0
            target_nicks = []
            trap_messages = []
            
            for target_uid_str in interacting_users:
                # 失去10%~30%道具卡（至少1张）
                target_items = item_data.get(today, {}).get(target_uid_str, [])
                actual_lost = 0
                lost_items = []
                if target_items:
                    pct = random.randint(10, 30) / 100.0
                    num_to_lose = max(1, int(len(target_items) * pct))
                    actual_lost = min(len(target_items), num_to_lose)
                    for _ in range(actual_lost):
                        if target_items:
                            lost_items.append(target_items.pop(random.randrange(len(target_items))))
                    lost_items_total += actual_lost
                    
                    # 触发陷阱卡检查
                    trap_msg = self._check_and_trigger_trap(today, target_uid_str, uid, lost_items, gid)
                    if trap_msg:
                        trap_messages.append(trap_msg)
                
                # 失去1-3个老婆
                target_wives = cfg.get(target_uid_str, {}).get("wives", [])
                actual_wife_lost = 0
                lost_wives_list = []
                if target_wives:
                    num_wives_to_lose = random.randint(1, 3)
                    actual_wife_lost = min(len(target_wives), num_wives_to_lose)
                    for _ in range(actual_wife_lost):
                        lost_wives_list.append(target_wives.pop(random.randrange(len(target_wives))))
                    
                    if len(target_wives) == 0:
                        del cfg[target_uid_str]
                    else:
                        cfg[target_uid_str]["wives"] = target_wives
                        if not get_user_flag(today, target_uid_str, "harem"):
                            cfg[target_uid_str]["harem"] = False
                            
                    lost_wives_total += actual_wife_lost
                    # 流失通知
                    self._handle_wife_loss(today, target_uid_str, actual_wife_lost, gid, lost_wives_list=lost_wives_list)
                
                # 获取昵称
                target_info = cfg.get(target_uid_str, {})
                target_nick = target_info.get("nick", f"用户{target_uid_str}") if isinstance(target_info, dict) else f"用户{target_uid_str}"
                target_nicks.append(target_nick)
            
            save_item_data()
            save_group_config(cfg)
            
            targets_str = "..".join(target_nicks) if len(target_nicks) > 1 else target_nicks[0]
            # 为了跟上面兼容：
            targets_str = "、".join(target_nicks)
            result = f"报仇雪恨！你让 {targets_str} 随机失去了道具卡和老婆。共计失去 {lost_items_total} 张道具卡和 {lost_wives_total} 个老婆。"
            if trap_messages:
                result += "".join(trap_messages)
            return await finalize(True, result)
        if name == "神的不在场证明":
        ...
        if name == "发薪日":
            ...
            for u in cfg.keys():
                u_str = str(u)
                add_user_mod(today, u_str, "blind_box_extra_draw", 1 * double_factor)
                if u_str != uid:
                    others_count += 1
        ...
        if name == "博弈":
            ...
            # 自己随机 +/- 1~50%（受二度寝影响翻倍）
            my_bonus = (random.randint(1, 50) / 100.0) * double_factor
            if get_user_flag(today, uid, "cheat"):
                # 老千必定获得正向加成
                pass
            else:
            ...
            # 目标随机 +/- 1~50%
            target_bonus = random.randint(1, 50) / 100.0
            if random.random() < 0.5:
                target_bonus = -target_bonus
            
            # 存储加成 (1 + bonus)
            # 使用 meta 存储
            set_user_meta(today, uid, "gamble_multiplier", 1.0 + my_bonus)
            set_user_meta(today, target_uid_str, "gamble_multiplier", 1.0 + target_bonus)
            
            my_type = "增加" if my_bonus > 0 else "减少"
            target_type = "增加" if target_bonus > 0 else "减少"
            
            result = f"博弈开始！\n你的概率加成{my_type}了 {abs(int(my_bonus*100))}%；\n目标概率加成{target_type}了 {abs(int(target_bonus*100))}%。"
            return await finalize(True, result, double_effect_used=double_active and double_factor > 1)
        if name == "幕后交易":
            today_items = item_data.setdefault(today, {})
            user_items = today_items.get(uid, [])
            lost_count = len(user_items)
            if lost_count <= 0:
                return await finalize(False, "你目前没有任何道具卡可以进行幕后交易。")
            user_items.clear()
            save_item_data()
            ...
            new_labels = []
            for state_item in drawn:
                spec = next((s for s in self.status_effect_specs if s.get("item_name") == state_item), {})
                new_labels.append(spec.get("label", state_item))
                await self.apply_item_effect(state_item, event, None, caller_uid=uid, use_double_effect=False, consume_double_effect=False, skip_stacking_tower=True)
                
            if new_labels:
                return await finalize(True, f"幕后交易完成！你失去了 {lost_count} 张道具卡，获得了状态：{'、'.join(new_labels)}。", double_effect_used=double_active and double_factor > 1)
            else:
                return await finalize(True, f"幕后交易完成！你失去了 {lost_count} 张道具卡，但很遗憾没有获得新状态。")
        if name == "多重人格":
            set_user_flag(today, uid, "multiple_personalities", True)
            return await finalize(True, f"你获得了「多重人格」状态！每隔2小时将你所有的状态及道具卡重新随机。")
        if name == "敏感肌":
            set_user_flag(today, uid, "sensitive_skin", True)
            return await finalize(True, f"你获得了「敏感肌」状态！当别人对你使用道具卡或指令时，你将自动反击！")
        if name == "过期盲盒":
            add_user_mod(today, uid, "blind_box_extra_draw", 1 * double_factor)
            msg = f"你打开了过期盲盒，获得了{1 * double_factor}次额外抽盲盒机会。"
            if random.random() < 0.75:
                # 确定惩罚类型：根据实际拥有的老婆和道具卡来决定，避免随机到空项
                wives = cfg.get(uid, {}).get("wives", [])
                today_items = item_data.setdefault(today, {})
                user_items = today_items.get(uid, [])
                
                lose_type = None
                if wives and user_items:
                    lose_type = random.choice(["wife", "item"])
                elif wives:
                    lose_type = "wife"
                elif user_items:
                    lose_type = "item"
                ...
                if lose_type == "wife":
                    num_to_lose = random.randint(1, min(3, len(wives)))
                    lost_wives = []
                    for _ in range(num_to_lose):
                        lost_wives.append(wives.pop(random.randrange(len(wives))))
                    save_group_config(cfg)
                    self._handle_wife_loss(today, uid, num_to_lose, gid, lost_wives_list=lost_wives)
                    wife_names = "、".join([self._get_wife_display_name(cfg, w['name'] if isinstance(w, dict) else w) for w in lost_wives])
                    msg += f"\n糟糕！过期盲盒有毒，你失去了老婆：{wife_names}。"
                elif lose_type == "item":
                    pct = random.randint(10, 50) / 100.0
                    lose_count = max(1, int(len(user_items) * pct))
                    lose_count = min(lose_count, len(user_items))
                    lost_items = []
                    for _ in range(lose_count):
                        lost_items.append(user_items.pop(random.randrange(len(user_items))))
                    save_item_data()
                    lost_items_text = "、".join(lost_items)
                    msg += f"\n糟糕！过期盲盒爆炸了，你失去了道具卡：{lost_items_text}。"
            return await finalize(True, msg, double_effect_used=double_active and double_factor > 1)
        if name == "安慰奖":
            choice = random.choice(["item", "status", "wife"])
            if choice == "item":
                drawn = self._draw_item_by_quality(today, uid, count=1 * double_factor, cfg=cfg, gid=gid)
                if drawn:
                    today_items = item_data.setdefault(today, {})
                    user_items = today_items.setdefault(uid, [])
                    user_items.extend(drawn)
                ...
                if drawn:
                    new_labels = []
                    for state in drawn:
                        await self.apply_item_effect(state, event, None, caller_uid=uid, use_double_effect=False, consume_double_effect=False, skip_stacking_tower=True)
                        spec = next((s for s in self.status_effect_specs if s.get("item_name") == state), {})
                        new_labels.append(spec.get("label", state))
                    states_text = "、".join(new_labels)
                    return await finalize(True, f"安慰奖：你获得了状态「{states_text}」！", double_effect_used=double_active and double_factor > 1)
            ...
            if img:
                ...
                if add_wife(cfg, uid, img, today, nick, is_harem):
                    save_group_config(cfg)
                    wife_name = self._get_wife_display_name(cfg, img)
                    return await finalize(True, f"安慰奖：你获得了老婆「{wife_name}」！")
            return await finalize(True, f"安慰奖：卡池空空如也，什么都没获得...")
        if name == "爱马仕":
            # 设置爱马仕状态
        ...
        if name == "trace-on":
            ...
            if bonus_card:
                if bonus_card.endswith("[FAKE]"):
                    return await finalize(True, f"trace on——魔术回路启动！今天当你手中没有道具卡时会自动获得随机道具卡（最多20次）。魔术回路立即发动，你获得了赝品...。")
                else:
                    return await finalize(True, f"trace on——魔术回路启动！今天当你手中没有道具卡时会自动获得随机道具卡（最多20次）。魔术回路立即发动，你获得了「{bonus_card}」。")
            return await finalize(True, f"trace on——你获得了「魔术回路」状态！今天当你手中没有道具卡时会自动获得随机道具卡（最多20次）。")
        if name == "55开":
            # 设置众生平等状态
        ...
        if name == "囤囤鼠":
            ...
            return await finalize(True, f"你成为了盲盒爱好者！今日每2小时可以免费抽一次盲盒，但每3分钟只能使用一个道具。")
        if name == "会员制餐厅":
            ...
            return await finalize(True, f"你获得了{3 * double_factor}次额外赠送指令次数，并获得了「淳平」状态！", double_effect_used=double_active and double_factor > 1)
        if name == "柏青哥":
            # 随机触发一种效果
            ...
            if choice == "reset_and_gain":
                ...
                return await finalize(
                    ...
                    double_effect_used=double_active and double_factor > 1
                )
                ...
                return await finalize(True, f"777！你获得{bonus_draws}次额外抽盲盒次数，且今日抽盲盒必定触发一次暴击。", double_effect_used=double_active and double_factor > 1)
        ...
        if name == "洗牌":
            ...
            # 50%~150%数量的道具卡，"老千"状态下为75%~175%
            pct = random.randint(50, 150) / 100.0 if not has_cheat else random.randint(75, 175) / 100.0
            item_count = max(1, round(base_item_count * pct))
            # 清空所有道具
            ...
            today_items[uid] = []
            save_item_data()
            # 从道具池中随机抽取指定数量的道具，降低获得"洗牌"的概率
            weights = [0.1 if item == "洗牌" else 1.0 for item in self.item_pool]
            new_items = random.choices(self.item_pool, weights=weights, k=item_count)
        ...
        if name == "支配恶魔":
            ...
            return await finalize(True, f"你获得了「支配恶魔」状态！获得了{2 * double_factor}次换老婆额外次数，现在你可以在换老婆时@目标用户，让目标使用换老婆指令（消耗你的次数，且次数消耗翻倍）。", double_effect_used=double_active and double_factor > 1)
        if name == "奇迹于你":
            # 获得"奇迹于你"状态
        ...
        if name == "情敌":
            ...
            if target_uid in cfg:
                ...
                save_group_config(cfg)
            fortune_msg = self._handle_wife_loss(today, target_uid, 1, gid, lost_wives_list=[lost_wife])
            target_info = cfg.get(target_uid, {})
            target_nick = target_info.get("nick", f"用户{target_uid}") if isinstance(target_info, dict) else f"用户{target_uid}"
        ...
        if name == "离婚":
            ...
            # 先保存失去的老婆列表
            lost_wives_list = list(user_wives) if user_wives else []
            if uid in cfg:
                del cfg[uid]
                save_group_config(cfg)
        ...
        if name == "出千":
            # 20%概率获得"老千"状态，或者将所有道具提升一个品质
            today_items = item_data.setdefault(today, {})
            user_items = today_items.get(uid, [])
            
            if self._probability_check(0.2, today, uid, positive=True):
        ...
        if name == "献上忠诚":
            if not target_uid:
                return await finalize(False, f"使用「献上忠诚」时请@目标用户哦~")
            target_uid = str(target_uid)
            if target_uid == uid:
                return await finalize(False, f"不能对自己使用「献上忠诚」哦~")
            ...
            if not user_has_equal_rights and not target_has_equal_rights:
                if get_user_flag(today, target_uid, "ban_items"):
                    return await finalize(False, f"对方正处于贤者时间，无法对其使用「献上忠诚」。")
            else:
                if get_user_flag(today, target_uid, "ban_items"):
                    if user_has_equal_rights:
                        equal_rights_msg = "众生平等！你无视了对方的贤者时间，"
            ...
            target_items = list(today_items.get(target_uid, []))
            if not target_items:
                return await finalize(False, f"对方口袋里空空如也，没有道具卡可以被献上忠诚。")
            
            pct = random.randint(10, 50) / 100.0
            lose_count = max(1, round(len(target_items) * pct * double_factor))
            lose_count = min(lose_count, len(target_items))
            
            # 抽取抢走的卡
            stolen_items = []
            for _ in range(lose_count):
                stolen_items.append(target_items.pop(random.randrange(len(target_items))))
            today_items[target_uid] = target_items
            save_item_data()
            
            # 损失通知
            target_fortune_msg = self._handle_item_loss(today, target_uid, lose_count, gid)
            
            # 获取其他群友列表
            others = [str(u) for u in cfg.keys() if str(u) != target_uid]
            if not others:
                others = [target_uid]
                
            member_counts = {}
            for m in others:
                member_counts[m] = len(today_items.get(m, []))
                
            distribution = {}
            for item in stolen_items:
                min_count = min(member_counts.values())
                candidates = [m for m, count in member_counts.items() if count == min_count]
                recipient = random.choice(candidates)
                distribution.setdefault(recipient, []).append(item)
                member_counts[recipient] += 1
                
            for r_uid, items in distribution.items():
                today_items.setdefault(r_uid, []).extend(items)
            save_item_data()
            
            # 拼接分配明细
            dist_details = []
            for r_uid, items in distribution.items():
                r_info = cfg.get(r_uid, {})
                r_nick = r_info.get("nick", f"用户{r_uid}") if isinstance(r_info, dict) else f"用户{r_uid}"
                dist_details.append(f"{r_nick}（{len(items)}张）")
            
            dist_text = "、".join(dist_details)
            target_info = cfg.get(target_uid, {})
            target_nick = target_info.get("nick", f"用户{target_uid}") if isinstance(target_info, dict) else f"用户{target_uid}"
            
            trap_msg = self._check_and_trigger_trap(today, target_uid, uid, stolen_items, gid)
            
            msg = f"{equal_rights_msg}献上忠诚！{target_nick} 失去了 {lose_count} 张道具卡，并被随机分配给了：{dist_text}。"
            if trap_msg:
                msg += trap_msg
            if target_fortune_msg:
                msg += f"\n{target_nick}{target_fortune_msg.replace('祸福相依：', '的祸福相依：')}"
        ...
        if name == "发狂":
            if not target_uid:
                return await finalize(False, f"使用「发狂」时请@目标用户哦~")
            target_uid = str(target_uid)
            if target_uid == uid:
                return await finalize(False, f"不能对自己使用「发狂」哦~")
                
            # 过滤出单目标道具
            single_target_items = [
                item for item in self.items_need_target
                if item not in {"月老", "发狂"}
            ]
            transformed_card = random.choice(single_target_items)
            
            # 触发变身卡效果
            success, transformed_msg = await self.apply_item_effect(
                transformed_card,
                event,
                target_uid,
                extra_arg,
                caller_uid=uid,
                use_double_effect=use_double_effect,
                consume_double_effect=consume_double_effect,
                skip_stacking_tower=True
            )
            
            if transformed_msg:
                transformed_msg = f"发狂发动！变身为了「{transformed_card}」：\n{transformed_msg}"
            return success, transformed_msg

        if name == "广告牌":
            # 过滤出无目标道具
            no_target_items = [
                item for item in self.item_pool
                if item not in self.items_need_target and item != "广告牌"
            ]
            transformed_card = random.choice(no_target_items)
            ...
            if transformed_msg:
                transformed_msg = f"广告牌翻转！变身为了「{transformed_card}」：\n{transformed_msg}"
            return success, transformed_msg
        # 其他未实现
    ...
    async def animewife(self, event: AstrMessageEvent, free_draw: bool = False):
        ...
        if is_harem:
            ...
            if not free_draw:
                ...
                if change_rec.get("count", 0) >= max_draw:
                    yield event.plain_result(f"{event.get_sender_name()}，你今天已经抽了{max_draw}次老婆啦（开后宫模式下抽老婆次数=换老婆次数），明天再来吧~")
                    return
                change_records[uid] = change_rec
            # 检查是否已有老婆（开后宫可以继续抽）
            wife_count = get_wife_count(cfg, uid, today)
                    ...
                    if self._probability_check(prob, today, uid, positive=False):
                        ...
                        def shura_omen_handler(today, target_uid, gid):
                            ...
                            if target_uid in target_cfg:
                                ...
                                # 先保存失去的老婆列表
                                lost_wives_list = list(get_wives_list(target_cfg, target_uid, today)) if loss > 0 else []
                                del target_cfg[target_uid]
                                save_group_config(target_cfg)
                                self._handle_wife_loss(today, target_uid, loss, gid, lost_wives_list=lost_wives_list)
                        ...
                        if not get_user_flag(today, uid, "omen_pioneer"):
                            ...
                            if uid in cfg:
                                ...
                                del cfg[uid]
                                save_group_config(cfg)
                                fortune_msg = self._handle_wife_loss(today, uid, loss, gid, lost_wives_list=lost_wives_list)
                            add_user_mod(today, uid, "change_extra_uses", 1)
                            ...
                            add_user_mod(today, uid, "change_extra_uses", 1)
                            yield event.plain_result(f"{event.get_sender_name()}，修罗场爆发！但奇迹于你效果发动，事件已转移，你获得了一次抽老婆的机会。")
                        return
        else:
            # 普通用户：今天已抽则直接返回
            ...
            if should_filter:
                ...
                if len(status_filters) > 1:
                    ...
                    if not filtered_pool:
                        yield event.plain_result(f"{event.get_sender_name()}，抱歉，在同时拥有{'+'.join(status_names)}状态的情况下，没有找到满足条件的角色，请稍后再试~")
                        return
        ...
        # 检查情敌状态
        rival_stolen = False
        rival_stolen_wife_name = ""
        rival_target_nick = ""
        
        if get_user_flag(today, uid, "rival"):
            rival_target = get_user_meta(today, uid, "rival_target")
            if rival_target:
                rival_target_str = str(rival_target)
                target_wives = list(get_wives_list(cfg, rival_target_str, today))
                if target_wives:
                    # 计算抢夺概率（受运势影响）
                    rival_prob = self._calculate_probability(0.35, today, uid, gain_multiplier=1.0)
                    rival_prob = min(0.9, rival_prob)
    ...
    async def ntr_wife(self, event: AstrMessageEvent):
        ...
        if get_user_flag(today, uid, "protect_from_ntr") and not get_user_flag(today, uid, "ntr_override"):
            yield event.plain_result(f"{event.get_sender_name()}，纯爱战士不会使用「牛老婆」指令哦~")
            return
        # 检查使用者是否拥有公交车效果（无法使用牛老婆）
        if get_user_flag(today, uid, "ban_ntr") and not get_user_flag(today, uid, "ntr_override"):
            yield event.plain_result(f"{event.get_sender_name()}，公交车效果：你今天无法使用「牛老婆」指令哦~")
            return
        # 检查是否为所欲为状态
        ...
        if is_do_whatever:
            ...
            if len(ntr_timestamps) >= 3:
                ...
                if wait_minutes > 0:
                    ...
                    wait_text = f"{wait_secs}秒"
                yield event.plain_result(f"{event.get_sender_name()}，为所欲为状态下，10分钟内只能使用3次「牛老婆」，请等待{wait_text}后再试~")
                return
            # 记录本次使用时间（在通过所有检查后，在成功时记录）
        else:
            # 普通状态：检查每日次数限制
            if rec["count"] >= max_ntr:
                yield event.plain_result(f"{event.get_sender_name()}，你今天已经牛了{max_ntr}次啦，明天再来吧~"
                )
                return
        riddler_messages = []
        riddler_messages = []
        tid = self.parse_target(event, riddler_messages=riddler_messages)
        if not tid or tid == uid:
            msg = "请@你想牛的对象哦~" if not tid else "不能牛自己呀，换个人试试吧~"
            if riddler_messages:
                msg = f"{' '.join(riddler_messages)}\n{msg}"
            yield event.plain_result(f"{event.get_sender_name()}，{msg}")
            return
        cfg = load_group_config(gid)
        # 检查目标是否有老婆（支持开后宫用户）
        ...
        if forced_success or random.random() < final_prob:
            ...
            if landmine_girl and attacker_has_wife:
                ...
                if event_choice == "kill_wife":
                    ...
                    yield event.plain_result(f"{event.get_sender_name()}，你的老婆{murderer_name}不小心把{victim_name}杀了......")
                    return
                    ...
                    try:
                        ...
                        pass
                    yield event.plain_result(f"{event.get_sender_name()}，你被你的老婆打晕了......但好消息是，你还活着")
                    return
                    ...
                    def suicide_omen_handler(today, target_uid, gid):
                        ...
                        if target_uid in target_cfg:
                            # 先保存失去的老婆列表
                            lost_wives_list = list(get_wives_list(target_cfg, target_uid, today)) if target_loss > 0 else []
                            del target_cfg[target_uid]
                            save_group_config(target_cfg)
                            self._handle_wife_loss(today, target_uid, target_loss, gid, lost_wives_list=lost_wives_list)
                    ...
                    if not get_user_flag(today, uid, "omen_pioneer"):
                        ...
                        yield event.plain_result(f"{event.get_sender_name()}，你的老婆自杀了......但奇迹于你效果发动，事件已转移。")
                    return
            ...
            dingdong_msg = await self._handle_dingdong_effect(today, uid, tid, gid)
            if dingdong_msg:
                yield event.plain_result(dingdong_msg)
            ...
            equal_rights_msg = equal_rights_prefix if 'equal_rights_prefix' in locals() else ""
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n{equal_rights_msg}很遗憾，牛失败了！你今天还可以再试{rem}次~" if riddler_info else f"{equal_rights_msg}很遗憾，牛失败了！你今天还可以再试{rem}次~"
            yield event.plain_result(result_msg)
        ...
        if is_harem_user(cfg, tid):
            ...
            if not wives:
                if is_self:
                    ...
                    yield event.plain_result(f"{event.get_sender_name()}，{owner}今天还没有老婆哦~")
                return
            img = wives[0]  # 普通用户只有一个老婆
            if img.startswith("http"):
                display_name = self._resolve_avatar_nick(cfg, img)
    ...
    async def switch_ntr(self, event: AstrMessageEvent):
        ...
        if uid not in self.admins:
            yield event.plain_result(f"{event.get_sender_name()}，你没有权限操作哦~")
            return
        ntr_statuses[gid] = not ntr_statuses.get(gid, False)
        save_ntr_statuses()
        load_ntr_statuses()
        state = "开启" if ntr_statuses[gid] else "关闭"
        yield event.plain_result(f"{event.get_sender_name()}，NTR已{state}")
    ...
    async def change_wife(self, event: AstrMessageEvent):
        ...
        if is_dominate_demon:
            ...
            if target_uid:
                ...
                target_wives = get_wives_list(cfg, target_uid, today)
                if not target_wives:
                    yield event.plain_result(f"{event.get_sender_name()}，目标用户{target_nick}今天还没有老婆，无法使用支配恶魔效果。")
                    return
                # 检查目标是否开后宫
                if get_user_flag(today, target_uid, "harem"):
                    yield event.plain_result(f"{event.get_sender_name()}，目标用户{target_nick}处于开后宫状态，无法使用「换老婆」指令哦~")
                    return
                ...
                if required_count > max_change:
                    yield event.plain_result(f"{event.get_sender_name()}，你的换老婆次数不足（支配恶魔效果需要消耗2次），无法使用。")
                    return
                # 执行目标的换老婆逻辑
                ...
                yield event.plain_result(f"{event.get_sender_name()}，支配恶魔效果：你让{target_nick}使用了换老婆指令（消耗了你2次换老婆次数）")
                # 触发左右开弓等效果（以目标身份）
        ...
        if get_user_flag(today, uid, "protect_from_ntr") or get_user_flag(today, uid, "next_ntr_guarantee"):
            yield event.plain_result(f"{event.get_sender_name()}，你今天无法使用「换老婆」哦~")
            return
        # 开后宫模式：无法使用换老婆指令
        is_harem = get_user_flag(today, uid, "harem")
        if is_harem:
            yield event.plain_result(f"{event.get_sender_name()}，开后宫状态下无法使用「换老婆」指令哦~")
            return
        # 检查是否为所欲为状态
        is_do_whatever = get_user_flag(today, uid, "do_whatever")
        if is_do_whatever:
            ...
            if len(change_timestamps) >= 3:
                ...
                if wait_minutes > 0:
                    ...
                    wait_text = f"{wait_secs}秒"
                yield event.plain_result(f"{event.get_sender_name()}，为所欲为状态下，10分钟内只能使用3次「换老婆」，请等待{wait_text}后再试~")
                return
            # 记录本次使用时间（在通过所有检查后，在成功时记录）
        else:
            # 普通状态：检查每日次数限制
            ...
            if is_dominate_demon:
                ...
                if check_count + 2 > max_change:
                    yield event.plain_result(f"{event.get_sender_name()}，你今天已经换了{max_change}次老婆啦，明天再来吧~（支配恶魔效果：每次消耗2次）"
                    )
                    return
            else:
                if check_count >= max_change:
                    yield event.plain_result(f"{event.get_sender_name()}，你今天已经换了{max_change}次老婆啦，明天再来吧~"
                    )
                    return
        wives = get_wives_list(cfg, uid, today)
        if not wives:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有老婆，先去抽一个再来换吧~")
            return
        lost_count = len(wives)
        # 删除旧老婆数据（先保存失去的老婆列表）
        lost_wives_list = list(wives) if wives else []
        if uid in cfg:
            del cfg[uid]
        ...
        if consume:
            self._grant_lightbulb_bonus(today, gid, "change")
        if free_msg:
            yield event.plain_result(f"{event.get_sender_name()}，{nick}{free_msg}")
    ...
    async def reset_ntr(self, event: AstrMessageEvent):
        ...
        today = get_today()
        if get_user_flag(today, uid, "lightbulb"):
            yield event.plain_result(f"{event.get_sender_name()}，电灯泡状态下无法使用「重置牛」指令哦~")
            return
        # 开后宫用户无法使用重置指令
        if get_user_flag(today, uid, "harem"):
            yield event.plain_result(f"{event.get_sender_name()}，开后宫状态下无法使用“重置牛”指令哦~")
            return
    ...
    async def reset_change_wife(self, event: AstrMessageEvent):
        ...
        today = get_today()
        if get_user_flag(today, uid, "lightbulb"):
            yield event.plain_result(f"{event.get_sender_name()}，电灯泡状态下无法使用「重置换」指令哦~")
            return
        # 开后宫用户无法使用重置指令
        if get_user_flag(today, uid, "harem"):
            yield event.plain_result(f"{event.get_sender_name()}，开后宫状态下无法使用“重置换”指令哦~")
            return
        if uid in self.admins:
            tid = self.parse_at_target(event, ignore_zero_attention=True, ignore_riddler=True) or uid
    ...
    async def swap_wife(self, event: AstrMessageEvent):
        ...
        uid = str(event.get_sender_id())
        riddler_messages = []
        tid = self.parse_at_target(event, riddler_messages=riddler_messages)
        nick = event.get_sender_name()
        today = get_today()
        # 检查发起者是否拥有开后宫效果
        if get_user_flag(today, uid, "harem"):
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n开后宫状态下无法使用「交换老婆」指令哦~" if riddler_info else f"开后宫状态下无法使用「交换老婆」指令哦~"
            yield event.plain_result(result_msg)
            return
        # 检查发起者是否拥有纯爱战士效果
        if get_user_flag(today, uid, "protect_from_ntr"):
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n纯爱战士不会使用「交换老婆」指令哦~" if riddler_info else f"纯爱战士不会使用「交换老婆」指令哦~"
            yield event.plain_result(result_msg)
            return
        ...
        if rec_lim["count"] >= self.swap_max_per_day:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n你今天已经发起了{self.swap_max_per_day}次交换请求啦，明天再来吧~" if riddler_info else f"你今天已经发起了{self.swap_max_per_day}次交换请求啦，明天再来吧~"
            yield event.plain_result(result_msg)
            return
        if not tid or tid == uid:
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n请在命令后@你想交换的对象哦~" if riddler_info else f"请在命令后@你想交换的对象哦~"
            yield event.plain_result(result_msg)
            return
        # 众生平等：无视目标状态（使用者有众生平等 或 目标有众生平等）
        user_has_equal_rights = get_user_flag(today, uid, "equal_rights")
        ...
        if not user_has_equal_rights and not target_has_equal_rights:
            ...
            if get_user_flag(today, tid, "harem"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n无法对开后宫状态的用户使用「交换老婆」指令哦~" if riddler_info else f"无法对开后宫状态的用户使用「交换老婆」指令哦~"
                yield event.plain_result(result_msg)
                return
            # 检查目标是否拥有纯爱战士效果
            if get_user_flag(today, tid, "protect_from_ntr"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n无法对纯爱战士使用「交换老婆」指令哦~" if riddler_info else f"无法对纯爱战士使用「交换老婆」指令哦~"
                yield event.plain_result(result_msg)
                return
        else:
            # 众生平等状态：豁免保护，收集提示语（不单独发送）
        ...
        for x in (uid, tid):
            ...
            if wife_count == 0:
                ...
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n{who}，今天还没有老婆，无法进行交换哦~" if riddler_info else f"{who}，今天还没有老婆，无法进行交换哦~"
                yield event.plain_result(result_msg)
                return
        # 检查是否有公交车效果（强制交换）
        ...
        if force_swap:
            ...
            if wives_u and wives_t:
                ...
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n{swap_equal_rights_msg}公交车效果发动！强制交换成功！" if riddler_info else f"{swap_equal_rights_msg}公交车效果发动！强制交换成功！"
                yield event.plain_result(result_msg)
                if cancel_msg:
                    yield event.plain_result(cancel_msg)
                return
    ...
    async def agree_swap_wife(self, event: AstrMessageEvent):
        ...
        riddler_messages = []
        uid = self.parse_at_target(event, riddler_messages=riddler_messages)
        nick = event.get_sender_name()
        today = get_today()
        ...
        if not agreeing_has_equal_rights:
            ...
            if get_user_flag(today, tid, "harem"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n开后宫状态下无法使用「同意交换」指令哦~" if riddler_info else f"开后宫状态下无法使用「同意交换」指令哦~"
                yield event.plain_result(result_msg)
                return
            # 检查同意者是否拥有纯爱战士效果
            if get_user_flag(today, tid, "protect_from_ntr"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n纯爱战士不会使用「同意交换」指令哦~" if riddler_info else f"纯爱战士不会使用「同意交换」指令哦~"
                yield event.plain_result(result_msg)
                return
        else:
            # 众生平等状态：豁免保护，收集提示语（不单独发送）
        ...
        if not initiator_has_equal_rights:
            ...
            if get_user_flag(today, uid, "harem"):
                del grp[uid]
                save_swap_requests()
                yield event.plain_result(f"{event.get_sender_name()}，对方已开启后宫状态，无法进行交换哦~")
                return
            # 检查发起者是否拥有纯爱战士效果（可能在发起后使用了纯爱战士道具）
            if get_user_flag(today, uid, "protect_from_ntr"):
                del grp[uid]
                save_swap_requests()
                yield event.plain_result(f"{event.get_sender_name()}，对方已成为纯爱战士，无法进行交换哦~")
                return
        else:
            # 众生平等状态：豁免保护，收集提示语（不单独发送）
        ...
        for x in (uid, tid):
            ...
            if wife_count == 0:
                ...
                del grp[uid]
                save_swap_requests()
                yield event.plain_result(f"{event.get_sender_name()}，{who}，今天还没有老婆，无法进行交换哦~")
                return
        ...
        yield event.plain_result(f"{event.get_sender_name()}，{combined_equal_rights_msg}交换成功！你们的老婆已经互换啦，祝幸福~")
        if cancel_msg:
            yield event.plain_result(cancel_msg)
    ...
    async def reject_swap_wife(self, event: AstrMessageEvent):
        ...
        if not rejecting_has_equal_rights:
            if get_user_flag(today, tid, "ban_reject_swap"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n苦主无法使用「拒绝交换」指令哦~" if riddler_info else f"苦主无法使用「拒绝交换」指令哦~"
                yield event.plain_result(result_msg)
                return
        else:
            # 众生平等状态：豁免保护，收集提示语（不单独发送）
    ...
    async def select_wife(self, event: AstrMessageEvent):
        ...
        if uses <= 0:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有「选老婆」的使用次数，快去使用道具卡获得吧~")
            return
        # 解析关键词
        msg = event.message_str.strip()
        parts = msg.split(maxsplit=1)
        if len(parts) < 2:
            yield event.plain_result(f"{event.get_sender_name()}，请发送「选老婆 XXX」格式，XXX为关键词哦~")
            return
        keyword = parts[1].strip()
        if not keyword:
            yield event.plain_result(f"{event.get_sender_name()}，关键词不能为空哦~")
            return
        # 消耗使用次数
        ...
        if not filtered_imgs:
            if xianchong:
                yield event.plain_result(f"{event.get_sender_name()}，管人痴状态下，没有找到同时「{keyword}」和「Vtuber」的老婆，换个关键词试试吧~")
            else:
                yield event.plain_result(f"{event.get_sender_name()}，没有找到「{keyword}」的老婆，换个关键词试试吧~")
            return
        # 随机选择一个
        img = random.choice(filtered_imgs)
        # 检查是否开后宫
    ...
    async def beat_wife(self, event: AstrMessageEvent):
        ...
        if uses <= 0:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有「打老婆」的使用次数，快去使用道具卡获得吧~")
            return
        # 检查是否有老婆
        cfg = load_group_config(gid)
        wife_count = get_wife_count(cfg, uid, today)
        if wife_count == 0:
            yield event.plain_result(f"{event.get_sender_name()}，你还没有老婆，无法使用「打老婆」指令哦~")
            return
        # 消耗使用次数
        ...
        if has_landmine:
            ...
            yield event.plain_result(f"{event.get_sender_name()}，你打了{wife_name}...\n{text}")
        else:
            pain_lines = [
                "呜呜呜...为什么要这样对我...",
                "好痛...但是...如果是你的话...",
    ...
    async def seduce(self, event: AstrMessageEvent):
        ...
        if uses == 0:
            yield event.plain_result(f"{event.get_sender_name()}，你今天还没有「勾引」的使用次数，快去使用道具卡获得吧~")
            return
        # 检查是否无限使用（熊出没效果）
        is_unlimited = (uses == -1)
        ...
        if not user_has_equal_rights and not target_has_equal_rights:
            ...
            if get_user_flag(today, target_uid, "protect_from_ntr"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n对方是纯爱战士，不受勾引影响哦~" if riddler_info else "对方是纯爱战士，不受勾引影响哦~"
                yield event.plain_result(result_msg)
                return
            if get_user_flag(today, target_uid, "ban_items"):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n对方处于贤者时间，不受勾引影响哦~" if riddler_info else "对方处于贤者时间，不受勾引影响哦~"
                yield event.plain_result(result_msg)
                return
            if get_user_meta(today, target_uid, "competition_target", None):
                riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
                result_msg = f"{riddler_info}\n对方处于雄竞状态，不受勾引影响哦~" if riddler_info else "对方处于雄竞状态，不受勾引影响哦~"
                yield event.plain_result(result_msg)
                return
        else:
            # 众生平等状态：豁免保护，收集提示语（不单独发送）
        ...
        if len(user_records) >= 3:
            ...
            if wait_minutes > 0:
                ...
                wait_text = f"{wait_secs}秒"
            yield event.plain_result(f"{event.get_sender_name()}，你已力竭！请等待{wait_text}后再试~")
            return
        ...
        if is_unlimited:
            if self._probability_check(0.25, today, uid, positive=False):
                try:
                    ...
                    pass
                yield event.plain_result(f"{event.get_sender_name()}，勾引失败！你被禁言120秒......")
                return
        ...
        if random.random() < adjusted_prob:
            ...
            counter_msg = await self._trigger_sensitive_skin(event, today, gid, uid, target_uid)
            ...
            if cupid_msg:
                yield event.plain_result(cupid_msg)
            dingdong_msg = await self._handle_dingdong_effect(today, uid, target_uid, gid)
            if dingdong_msg:
                yield event.plain_result(dingdong_msg)
        else:
            seduce_equal_rights_msg = seduce_equal_rights_prefix if 'seduce_equal_rights_prefix' in locals() else ""
            riddler_info = "\n".join(riddler_messages) if riddler_messages else ""
            result_msg = f"{riddler_info}\n{seduce_equal_rights_msg}勾引失败！对方没有注意到你，下次再试试吧~" if riddler_info else f"{seduce_equal_rights_msg}勾引失败！对方没有注意到你，下次再试试吧~"
            yield event.plain_result(result_msg)
    ...
    async def reset_basics(self, event: AstrMessageEvent):
        ...
        # 检查管理员权限
        if uid not in self.admins:
            yield event.plain_result(f"{event.get_sender_name()}，仅管理员才能使用「重开」指令哦~")
            return
        # 解析目标（管理员指令需无视0人问你状态）
        target_uid = self.parse_at_target(event, ignore_zero_attention=True, ignore_riddler=True)
        if not target_uid:
            yield event.plain_result(f"{event.get_sender_name()}，请@你想清空数据的目标用户哦~")
            return
        target_uid = str(target_uid)
        # 先获取目标用户昵称（在清空数据之前）
        target_nick = "目标用户"
        ...
        if target_uid in cfg:
            # 先保存失去的老婆列表
            lost_wives_list = list(get_wives_list(cfg, target_uid, today)) if loss > 0 else []
            del cfg[target_uid]
            save_group_config(cfg)
            self._handle_wife_loss(today, target_uid, loss, gid, lost_wives_list=lost_wives_list)
        # 清空牛老婆记录（如果日期是今天）
        ...
        save_reset_blind_box_records()
        save_swap_limit_records()
        yield event.plain_result(f"{event.get_sender_name()}，已清空{target_nick}今日的所有数据（效果、道具、老婆、记录等）")
    ...
    def _calculate_probability(self, base_prob: float, today: str, uid: str, 
        ...
        # 博弈加成：位于第二乘区计算
        gamble_mult = get_user_meta(today, uid, "gamble_multiplier", 1.0)
        if gamble_mult != 1.0:
            gain_multiplier = gain_multiplier * float(gamble_mult)
    ...
    def _record_user_interaction(self, today: str, gid: str, uid: str, target_uid: str):
        ...
        if not target_uid or uid == target_uid:
            return
        # 检查神的不在场证明
        if get_user_flag(today, uid, "alibi"):
            return
        uid_str = str(uid)
        target_uid_str = str(target_uid)
        gid_str = str(gid)
    ...
    async def _trigger_sensitive_skin(self, event: AstrMessageEvent, today: str, gid: str, caller_uid: str, target_uid: str) -> str:
        """
        触发敏感肌效果：如果目标具有敏感肌状态，随机对攻击者反击一个指定目标的道具。
        返回反击的提示信息，如果没有触发则返回空字符串。
        """
        caller_str = str(caller_uid)
        target_str = str(target_uid)
        if caller_str == target_str:
            return ""
        if not get_user_flag(today, target_str, "sensitive_skin"):
            return ""
        ...
        counter_item = random.choice(target_items)
        
        cfg = load_group_config(gid)
        target_info = cfg.get(target_str, {})
        target_nick = target_info.get("nick", f"用户{target_str}") if isinstance(target_info, dict) else f"用户{target_str}"
        
        counter_msg = f"\n【敏感肌触发】因为你的举动，触动了{target_nick}的「敏感肌」！对方掏出一张「{counter_item}」对你发起反击！\n反击："
        
        success, c_msg = await self.apply_item_effect(
            counter_item, 
            event, 
            caller_str,
            caller_uid=target_str,
    ...
    def _rival_desc_generator(self, today: str, uid: str, gid: str) -> str:
        target_uid = get_user_meta(today, uid, "rival_target")
        if target_uid:
            cfg = load_group_config(gid)
            target_info = cfg.get(str(target_uid), {})
            target_nick = target_info.get("nick", f"用户{target_uid}") if isinstance(target_info, dict) else f"用户{target_uid}"
            return f"情敌：与 {target_nick} 互为情敌"
        return "情敌：与他人互为情敌"

    def _check_and_trigger_trap(self, today: str, victim_uid: str, attacker_uid: str, lost_items: list, gid: str) -> str:
        """
        检查失去的道具中是否包含陷阱卡，如果包含，则惩罚攻击者使其失去10%~50%的道具卡
        """
        if not lost_items or "陷阱卡" not in lost_items:
            return ""
        if not attacker_uid or str(attacker_uid) == str(victim_uid):
            return ""
            
        attacker_str = str(attacker_uid)
        today_items = item_data.setdefault(today, {})
        attacker_items = today_items.get(attacker_str, [])
        if not attacker_items:
            return ""
            
        pct = random.randint(10, 50) / 100.0
        lose_count = max(1, int(len(attacker_items) * pct))
        lose_count = min(lose_count, len(attacker_items))
        
        stolen = []
        for _ in range(lose_count):
            stolen.append(attacker_items.pop(random.randrange(len(attacker_items))))
        today_items[attacker_str] = attacker_items
        save_item_data()
        
        # 记录流失通知
        self._handle_item_loss(today, attacker_str, lose_count, gid)
        
        # 获取昵称
        cfg = load_group_config(gid)
        attacker_info = cfg.get(attacker_str, {})
        attacker_nick = attacker_info.get("nick", f"用户{attacker_str}") if isinstance(attacker_info, dict) else f"用户{attacker_str}"
        victim_info = cfg.get(str(victim_uid), {})
        victim_nick = victim_info.get("nick", f"用户{victim_uid}") if isinstance(victim_info, dict) else f"用户{victim_uid}"
        
        return f"\n【陷阱卡触发】{victim_nick}失去的道具中包含「陷阱卡」！{attacker_nick}触发了陷阱，失去了 {lose_count} 张道具卡（{'、'.join(stolen)}）！"
    ...
    async def show_market(self, event: AstrMessageEvent):
        ...
        if today not in market_data or not market_data[today]:
            if not self._refresh_market(today):
                yield event.plain_result(f"{event.get_sender_name()}，集市暂时无法打开，请稍后再试~")
                return
    ...
    async def purchase_from_market(self, event: AstrMessageEvent):
        ...
        if not content:
            yield event.plain_result(f"{event.get_sender_name()}，请在「购买」后写上要购买的老婆名或道具名哦~")
            return
        ...
        if not purchased:
            yield event.plain_result(f"{event.get_sender_name()}，集市中没有找到「{content}」，请检查名称是否正确~")
            return

        if not self._consume_market_purchase_quota(today, uid, purchase_type, len(user_history)):
            yield event.plain_result(f"{event.get_sender_name()}，你今天的集市购买次数已经用完啦，明天再来吧~")
            return
        ...
        if purchase_type == "wife":
            ...
            if image_component:
                ...
                yield event.plain_result(f"{event.get_sender_name()}，购买成功！你获得了老婆：{name}")
        else:
            # 购买道具
            item_name = purchased
            today_items = item_data.setdefault(today, {})
            ...
            yield event.plain_result(f"{event.get_sender_name()}，购买成功！你获得了道具卡：{item_name}")
